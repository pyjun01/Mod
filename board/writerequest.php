    ...
    if($row->rowcount()>0){
        ...
        if(isset($_FILES)){
            ...
            */
            ...
            $sql= "INSERT INTO board_file SET file_name= '{$file_name}', file_extension='{$file_extension}', hash= '{$hash}', post_name='{$post_name}', down=0, date='{$date}'";
            $row= query($sql);
            if($row){
                echo "<script>location.replace('/{$post_idx}');</script>";
            }else{
